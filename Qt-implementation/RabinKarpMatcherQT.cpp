...
QVector<QString> RabinKarpMatcher::match(const Document& testDoc, const Corpus& corpus) {
            ...
            for (const auto& d : corpus.getDocuments()) {
                ...
                for (int i = 0; i <= d.getContent().size() - s.size(); i++) {
                    ...
                    if (i < d.getContent().size() - s.size()) {
                        dHash = ((dHash - static_cast<int>(s.at(i).toLatin1()) * powBase % prime) * base % prime + static_cast<int>(d.getContent().at(i).toLatin1()) + s.size()) % prime; // add modulus operation
...
int RabinKarpMatcher::calculateHash(const QString& str, int start, int end, int prime) {
   int hash = 0;
        ...
        for (int i = start; i <= end; i++) {
            hash = (hash + (static_cast<int>(str.at(i).toLatin1()) * powBase) % prime) % prime; // add modulus operation
            powBase = (powBase * base) % prime; // update power of base modulo prime
        }
        return hash;
    }
